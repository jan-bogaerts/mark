...
Only return the description, no introduction or explanation."""
...
def process_data(writer):
    for fragment in component_lister.text_fragments:
        ...
        for comp_name in fragment.data:
            comp_info = double_compress.get_fragment(fragment.full_title)
            if comp_info:
                params = {
...
def main(prompt, compressed, list, file=None):
    ...
    open_mode = 'w'
    if ONLY_MISSING:
        load_results(file + "_comp_descriptions.md")
        open_mode = 'a'

    print("rendering results")
    
    with open(file + "_comp_descriptions.md", open_mode) as writer:
        process_data(writer)
