...
def generate_response(params, key):
    ...
    total_tokens = 0
    model = get_model_config('component_lister', key)
    ...
    messages = []
    prompt = system_prompt.format(params['dev_stack'] ) + term_prompt
...
def process_data(writer):
    ...
    dev_stack = f'''{project.fragments[1].title}\n{project.fragments[1].content}'''
    ...
    for to_check in project.fragments[2:]:  # skip the first two fragments cause that's the description and dev stack
        ...
        if 'services' in to_check.full_title.lower():
            continue
        params = {
