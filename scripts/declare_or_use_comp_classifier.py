...
def generate_response(params, key):
    ...
    total_tokens = 0
    model = get_model_config('declare_or_use_comp', key)
    ...
    messages = []
    prompt = system_prompt.format(params['component'], params['comp_description'] )
...
def main(prompt, components_list, primary_list, comp_descr, double_comp, file=None):
    ...
    file_name = file + "_declare_or_use_comp.md"
    open_mode = 'w'
    if ONLY_MISSING:
