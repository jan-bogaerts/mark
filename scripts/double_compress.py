...
system_prompt = """condense the following text as much as possible, without loosing any meaning:"""
user_prompt = """{0}"""
...
def generate_response(params, key):
    ...
    total_tokens = 0
    model = get_model_config('double_compress', key)
...
def process_data(writer):
    ...
    for to_check in compress.text_fragments:  # skip the first two fragments cause that's the description and dev stack
...
def main(prompt, file=None):
    ...
    compress.load_results(prompt)

    # save there result to a file while rendering.
    if file is None:
        file = 'output'
    
    file_name = file + "_double_compress.md"
    open_mode = 'w'
    if ONLY_MISSING:
