...
async function buildMessage(textFragment) {
  ...
  var result = [
    {
      role: 'system',
      content: resources.component_lister_service_1.replace('{{dev_stack_title}}', services.projectService.textFragments[1]?.title).replace('{{dev_stack_content}}', services.projectService.textFragments[1]?.lines.join('\n')),
    },
    {
      role: 'user',
      content: resources.component_lister_service_2.replace('{{title}}', textFragment.title).replace('{{content}}', textFragment.lines.join('\n')),
    },
    {
      role: 'assistant',
      content: resources.component_lister_service_3,
    },
  ];
