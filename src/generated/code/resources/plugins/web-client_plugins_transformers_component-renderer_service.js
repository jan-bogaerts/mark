...
function getDescription() {
  return {
    ...
    isJson: false,
    description: 'the component-renderer service is responsible for generating all the code related to UI components.'
  };
}
...
async function buildMessage(fragment, component, components, renderToPath) {
  const result = [
    {
      role: 'system',
      content: resources.res_web_client_plugins_transformers_component_renderer_service_1.replace('{{name}}', component).replace('{{devStack}}', services.projectService.textFragments[1]?.lines.join('\n').trim()),
    },
    {
      role: "user",
    ...
    {
      role: 'assistant',
      content: resources.res_web_client_plugins_transformers_component_renderer_service_3,
    },
  ];
