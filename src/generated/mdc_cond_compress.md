...
- The project service is responsible for creating and opening projects, as well as saving them.
- When creating a new project, the service clears the cache, reloads data files for registered transformers, and raises a content-changed event.
- When opening an existing project, the service clears the cache, sets the file location, reads the file contents, parses the lines, reloads data files for registered transformers, and raises a content-changed event.
- When saving a project, the service moves or copies the file, writes the parsed objects to the file, and saves the filename.
- The service processes data changes made in the markdown editor.
- The service manages a data list of text fragments and provides functions to work with this list.
- The service keeps track of user configs, such as auto-save settings.
- The service uses events to notify others of changes, such as content changes, fragment deletions, fragment insertions, fragment out-of-date markings, and key changes.
...
- When a transformer calculates a result, it updates the cache dictionary.
- The cache also maintains a secondary dictionary to track relationships between text fragment keys.
- The cache stores results in a JSON file specified by the transformer.
- The cache registers event handlers with the project service to monitor changes in fragments.
- The cache can mark results as out of date and notify other cache services.
- The cache can overwrite results for a key and retrieve results for a key.
- The cache can check if a text fragment is out of date.
- The cache can be cleared using the clearCache() function.
- The cache can retrieve all results related to a fragment using the getFragmentResults() function.
- The cache handles fragment deletion and key changes using event handlers.
