...
The fragment-status icon component displays the status of a text-fragment object, including the icon, tooltip, and color, which are determined by the fragment's level and out-of-date status, and it also registers event handlers to refresh the icon and tooltip when certain events occur.
