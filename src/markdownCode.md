...
- handleLogMsg: convert the parameter from a json object to a string, add the string value to the components state field `logMsgs`
- the renderer of the log window loops over all the entries in logMsgs and displays the text. Make certain that:
  - the text is in a div that can be scrolled when the content is larger than the visible area.
  - the text is displayed using the styling `white-space: pre-wrap` so that new lines are rendered correctly
...
- functions:
  - beginMsg(transformerName, fragmentKey, inputData): 
    ...
    - convert the fragmentKey to a location using `keyService.calculateLocation(fragmentKey)`
    - store all the data and assigns a UUID to the log object. 
    - This log object is then serialized to a string and send to the log window using the global function `window.electron.logMsg`. 
